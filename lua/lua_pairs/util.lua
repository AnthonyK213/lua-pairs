...
---@class lua_pairs.Context
---@field p string The character before the cursor (previous);
---@field n string The character after the cursor  (next);
...
---@return lua_pairs.Context context
function M.get_ctxt()
  local context = {}
  ...
  context.f = fore
  if #back > 0 then
    local utfindex = vim.str_utfindex(back, _encoding)
    local s = vim.str_byteindex(back, _encoding, utfindex - 1)
    context.p = back:sub(s + 1, #back)
  else
    context.p = ""
  end
  if #fore > 0 then
    local e = vim.str_byteindex(fore, _encoding, 1)
